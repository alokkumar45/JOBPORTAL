...
export const register = catchAsyncErrors(async (req, res, next) => {
    try {
        ...
        if (!req.body || Object.keys(req.body).length === 0) {
            return next(new ErrorHandler("Request body is empty. Please send data as form-data.", 400));
        }
...
export const login = catchAsyncErrors(async (req, res, next) => {
    ...
    if (user.role !== role) {
        return next(new ErrorHandler(`User with provided role not found`, 400));
    }
